...
const extract = async (snomedCode: string) => {
  const { rows } = await query(`
    ...
    WHERE a.ancestor_id IN (
      SELECT source_id FROM snomed_relationship WHERE destination_id IN ('362965005', '64572001') AND type_id = '116680003' AND active = 1
    )
...
extract('195967001'); // Asthma
extract('38341003'); // Hypertensive disorder
