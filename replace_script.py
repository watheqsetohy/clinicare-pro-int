...
repl = '''                     <div className="bg-white border text-sm border-slate-200 rounded-xl p-4 shadow-sm relative flex flex-col">
                        <div className="flex bg-slate-100 p-1 rounded-lg mb-4 text-xs font-semibold">
                          <button onClick={() => setLogViewTab('chronological')} className={cn("flex-1 py-1.5 rounded-md flex items-center justify-center gap-2 transition-colors", logViewTab === 'chronological' ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-700")}>
...
target2 = '''                        ) : expandedConditionLogs.length > 0 ? (
...
repl2 = '''                        ) : expandedConditionLogs.length > 0 ? (() => {
                                ...
                                <div key={idx} className="relative pl-8">
                                  <div className={cn("absolute left-0 top-1 w-[24px] h-[24px] border-[3px] border-white rounded-full z-10 flex items-center justify-center", logViewTab === 'chronological' ? "bg-indigo-50" : "bg-slate-100")}>
                                    {logViewTab === 'chronological' ? (
                                      <div className={cn("w-2 h-2 rounded-full scale-125", log.isOnset ? "bg-indigo-600" : "bg-indigo-400")} />
                                    ) : (
                                      <div className={cn("w-2 h-2 rounded-full", log.isOnset ? "bg-indigo-600 scale-125" : log.action.includes('Active') ? "bg-emerald-500" : log.action.includes('Deactivate') ? "bg-amber-500" : "bg-blue-500")} />
                                    )}
                                  </div>
                                  <div className="text-[10px] text-slate-400 mb-0.5">
                                    {logViewTab === 'chronological' ? new Date(log.date).toLocaleString([], { dateStyle: 'medium'}) : new Date(log.system_date || log.date).toLocaleString()} • {log.isOnset ? 'Origin' : log.user || 'Clinician'}
                                  </div>
                                  <div className="text-sm font-semibold text-slate-800 leading-tight">{log.action}</div>
                                  {log.condition_term && (
                                    <div className={cn("text-[10px] uppercase mt-0.5", log.condition_code === flyoutCondition.snomed_code ? "text-blue-600 font-bold" : "font-bold text-slate-400")}>
